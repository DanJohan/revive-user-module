      ...
      <div class="navbar-nav ml-auto"> <a class="nav-item nav-link ts-scroll active" href="<?php echo base_url()."site/index"?>">Home <span class="sr-only">(current)</span></a> 
        ...
        <?php }?>
          ...
          <?php if($this->session->has_userdata('name')) { ?>
            <li class="nav-item dropdown"> <a class="nav-item nav-link ts-scroll nav-link dropdown-toggle" href="#" id="services" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Hi <?php echo ($this->session->has_userdata('name'))? ucfirst($this->session->userdata('name')) : '';?></a>
            <div class="dropdown-menu dp-clr" aria-labelledby="dropdown01"> 
