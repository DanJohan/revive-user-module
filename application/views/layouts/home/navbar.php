     ...
     <nav class="navbar navbar-expand-lg navbar-light navbar-bg fixed-top ts-separate-bg-element">
        ...
        <?php }}?>
         ...
         <?php if($this->session->has_userdata('name')) { ?>
            <li class="nav-item dropdown"> 
              <a class="nav-item nav-link ts-scroll nav-link dropdown-toggle" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Hi <?php echo ($this->session->has_userdata('name'))? ucfirst($this->session->userdata('name')) : '';?></a>
            <div class="dropdown-menu dp-clr" aria-labelledby="dropdown01"> 
...
<div id="ts-dynamic-waves" class="ts-background">
    <!-- s<svg class="ts-svg ts-z-index__1 ts-flip-x" width="100%" height="100%" version="1.1" xmlns="http://www.w3.org/2000/svg">
    <defs></defs>
